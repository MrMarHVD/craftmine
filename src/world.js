...
export class World {
  ...
  generateChunkData(cx, cz) {
    ...
    this.applyCastlesToChunk(blocks, cx, cz, worldX0, worldZ0);
  ...
  getCastleForRegion(regionX, regionZ) {
    const key = `${regionX},${regionZ}`;
    if (this.castleCache.has(key)) return this.castleCache.get(key);

    const spawnRoll = hash2D(regionX, regionZ, this.seed + 42031);
    if (spawnRoll > CASTLE_SPAWN_CHANCE) {
      this.castleCache.set(key, null);
      return null;
    }

    const rngSeed =
      ((regionX * 73856093) ^ (regionZ * 19349663) ^ (this.seed * 83492791) ^ 0x9e3779b9) | 0;
    ...
    const baseY = Math.min(
      ...
      1 +
        Math.max(
          this.getSurfaceYAt(baseX, baseZ),
          this.getSurfaceYAt(baseX + 8, baseZ),
          this.getSurfaceYAt(baseX, baseZ + 8),
          this.getSurfaceYAt(baseX + 8, baseZ + 8)
        )
    );
    ...
    for (let i = 0; i < targetPieces; i++) {
      ...
      const side = rng.range(0, 4); // 0 N, 1 E, 2 S, 3 W
  ...
  applyCastlesToChunk(blocks, cx, cz, worldX0, worldZ0) {
    ...
    for (let rz = minRegionZ; rz <= maxRegionZ; rz++) {
      for (let rx = minRegionX; rx <= maxRegionX; rx++) {
        const castle = this.getCastleForRegion(rx, rz);
        if (!castle) continue;
        const intersects =
        ...
        this.stampCastle(blocks, cx, cz, castle);
      }
    }
  }

  stampCastle(blocks, cx, cz, castle) {
    for (const piece of castle.pieces) {
      this.stampCastlePiece(blocks, cx, cz, castle.baseY, piece);
    }

    for (const t of castle.towers) {
      this.stampCastleTower(blocks, cx, cz, castle.baseY, t);
    }

    for (const c of castle.connections) {
      this.stampCastleConnection(blocks, cx, cz, castle.baseY, c);
    }
  }

  stampCastlePiece(blocks, cx, cz, baseY, piece) {
    ...
    for (let z = piece.z; z < piece.z + piece.d; z++) {
      for (let x = piece.x; x < piece.x + piece.w; x++) {
        this.setGeneratedBlockIfInChunk(blocks, cx, cz, x, baseY, z, BlockId.CASTLE_BRICK);

        for (let y = baseY + 1; y <= topY; y++) {
          const wall = x === piece.x || x === piece.x + piece.w - 1 || z === piece.z || z === piece.z + piece.d - 1;
          if (wall) this.setGeneratedBlockIfInChunk(blocks, cx, cz, x, y, z, BlockId.CASTLE_BRICK);
          else this.setGeneratedAirIfInChunk(blocks, cx, cz, x, y, z);
        }
        ...
        if (piece.kind !== "corridor") {
          ...
          if (edge || ((x + z) & 1) === 0) {
            this.setGeneratedBlockIfInChunk(blocks, cx, cz, x, roofY, z, BlockId.CASTLE_BRICK);
          } else {
            this.setGeneratedAirIfInChunk(blocks, cx, cz, x, roofY, z);
          }
        }
      }
    }
    ...
    if (piece.kind === "room" && piece.w >= 9 && piece.d >= 9) {
      ...
      for (const [cxp, czp] of cols) {
        for (let y = baseY + 1; y <= colY; y++) {
          this.setGeneratedBlockIfInChunk(blocks, cx, cz, cxp, y, czp, BlockId.CASTLE_BRICK);
        }
      }
    }
  }

  stampCastleTower(blocks, cx, cz, baseY, tower) {
    ...
    for (let z = tower.z - tower.r; z <= tower.z + tower.r; z++) {
      for (let x = tower.x - tower.r; x <= tower.x + tower.r; x++) {
        ...
        for (let y = baseY + 1; y <= topY; y++) {
          ...
          if (ring) this.setGeneratedBlockIfInChunk(blocks, cx, cz, x, y, z, BlockId.CASTLE_BRICK);
          else this.setGeneratedAirIfInChunk(blocks, cx, cz, x, y, z);
        }
        const crenelY = topY + 1;
        if (((x + z) & 1) === 0 || Math.max(dx, dz) === tower.r) {
          this.setGeneratedBlockIfInChunk(blocks, cx, cz, x, crenelY, z, BlockId.CASTLE_BRICK);
        }
      }
    }
  }

  stampCastleConnection(blocks, cx, cz, baseY, c) {
    for (let y = baseY + 1; y <= baseY + 4; y++) {
      for (let o = -1; o <= 1; o++) {
        if (c.side === 0) {
          this.setGeneratedAirIfInChunk(blocks, cx, cz, c.x + o, y, c.z);
          this.setGeneratedAirIfInChunk(blocks, cx, cz, c.x + o, y, c.z - 1);
        } else if (c.side === 1) {
          this.setGeneratedAirIfInChunk(blocks, cx, cz, c.x, y, c.z + o);
          this.setGeneratedAirIfInChunk(blocks, cx, cz, c.x + 1, y, c.z + o);
        } else if (c.side === 2) {
          this.setGeneratedAirIfInChunk(blocks, cx, cz, c.x + o, y, c.z);
          this.setGeneratedAirIfInChunk(blocks, cx, cz, c.x + o, y, c.z + 1);
        } else {
